...
public class HolidayController {
    ...
    public ResponseEntity<List<HolidayResponse>> getAllHolidays(
            ...
            @Parameter(description = "Filter by recurring holidays")
            @RequestParam(required = false) Boolean recurring,
            
            @Parameter(description = "Filter by name pattern")
            @RequestParam(required = false) String namePattern) {
        
        // Use DOP approach with HolidayData
        List<HolidayData> holidays = holidayService.findAll(country, state, city, type, startDate, endDate);
    ...
    @PostMapping
    @Operation(summary = "Create a new holiday", description = "Create a new holiday using DOP command pattern")
    ...
    public ResponseEntity<HolidayResponse> createHoliday(
        ...
        HolidayCommand.Create command = new HolidayCommand.Create(
            request.getName(),
            request.getDate(),
            location,
            request.getType(),
    ...
    @PutMapping("/{id}")
    @Operation(summary = "Update a holiday", description = "Update an existing holiday using DOP command pattern")
    ...
    public ResponseEntity<HolidayResponse> updateHoliday(
        ...
        if (request.getCountry() != null || request.getState() != null || request.getCity() != null) {
            location = Optional.of(new Location(request.getCountry(), request.getState(), request.getCity()));
        }
        
        HolidayCommand.Update command = new HolidayCommand.Update(
            id,
    ...
    @DeleteMapping("/{id}")
    @Operation(summary = "Delete a holiday", description = "Delete an existing holiday using DOP command pattern")
