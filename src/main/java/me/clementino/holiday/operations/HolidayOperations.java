...
/**
 * Pure operations on holiday data.
 * Following DOP v1.1 Principle 4: Separate Operations From Data.
 * 
 * This class contains only pure functions that work with immutable data.
 * No state is stored here - all operations are stateless and side-effect free.
 */
@Component
public class HolidayOperations {
    ...
    public String formatHolidayInfo(HolidayData holiday) {
        ...
        if (location.isNational()) {
            ...
            return "%s - State holiday in %s, %s".formatted(
                holiday.name(), location.state().get(), location.country());
        } else if (location.isCity()) {
            return "%s - Municipal holiday in %s, %s, %s".formatted(
    ...
    private ValidationResult validateMunicipalHoliday(HolidayData holiday) {
        if (!holiday.location().isCity()) {
            return new ValidationResult.Failure("Municipal holidays must have country, state, and city location");
        }
    ...
    private boolean matchesDateRange(HolidayData holiday, Optional<LocalDate> startDate, Optional<LocalDate> endDate) {
        LocalDate holidayDate = holiday.date();
